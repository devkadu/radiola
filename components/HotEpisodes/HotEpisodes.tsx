...
const tabs = ["Episódios quentes", "Mais comentados da semana", "Minhas séries"];
...
export const HotEpisodes = () => {
  ...
  const [simLogado, setSimLogado] = useState(false);

  const episodes = activeTab === 2 && simLogado ? mySeriesEpisodes : allEpisodes;

  return (
    <section className="px-4 lg:px-0 pt-4 pb-6">
      {/* Simular usuário logado */}
      <label className="flex items-center gap-2 text-xs text-[var(--text-muted)] mb-3 cursor-pointer w-fit">
        <input
          type="checkbox"
          checked={simLogado}
          onChange={(e) => setSimLogado(e.target.checked)}
          className="accent-[var(--yellow)]"
        />
        Simular usuário logado (aba Minhas séries)
      </label>
          ...
          return (
            ...
            >
              ...
              <div className="flex items-center gap-4">
                <span
                  className={`text-2xl font-bold w-6 shrink-0 leading-none ${
                    isFirst ? "text-black/30" : "text-[var(--text-muted)]"
                  }`}
                >
                  {ep.rank}
                </span>
                <div className="flex-1 min-w-0">
                  <p
                    className={`text-[11px] font-semibold uppercase tracking-widest mb-0.5 ${
                      isFirst ? "text-black/60" : "text-[var(--yellow)]"
                    }`}
                  >
                    {ep.series}
                  </p>
                  <p
                    className={`text-lg font-bold leading-tight ${
                      isFirst ? "text-black" : "text-[var(--text-primary)]"
                    }`}
                  >
                    {ep.episode}
                  </p>
                  <p
                    className={`text-xs mt-0.5 flex items-center gap-1.5 ${
                      isFirst ? "text-black/60" : "text-[var(--text-muted)]"
                    }`}
                  >
                    {ep.code}
                    <span
                      className={`w-1.5 h-1.5 rounded-full inline-block ${
                        isFirst ? "bg-black/40" : "bg-green-500"
                      }`}
                    />
                    {ep.debating} agora
                  </p>
              ...
              <div
                className={`flex items-center justify-between mt-2 pt-2 border-t ${
                  isFirst ? "border-black/20" : "border-[var(--border)]"
                }`}
              >
